...
const CheckoutForm = ({ setOrderCart }) => {
  ...
  useEffect(async () => {
    const formattedCart = cart.map(({ item, toppings, variant, quantity }) => {
      return {
        item: item.id,
    ...
    try {
      ...
      const { client_secret, amount } = paymentIntent;
  ...
  const stripeMutation = useMutation(async (CardElement) => {
  ...
  return <form onSubmit={handleSubmit} className="stack-l">
    <div className="px-2 py-4 border rounded-md">
      <CardElement options={stripeOptions}></CardElement>
    </div>
